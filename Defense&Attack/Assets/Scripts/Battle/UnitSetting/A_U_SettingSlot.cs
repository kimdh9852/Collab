...
public class A_U_SettingSlot : MonoBehaviour
{
    ...
    private Image ItemImg;  //유닛이 배치되면 나오는 이미지
    public GameObject Lock; // 엑스 이미지
    public bool isSlot;     // 현재 슬롯이 비어있는지?
    public string unit_Name; // 배치 유닛을 확인하기 위한 값. 나중에는 필요없을 수도 
    public int slotNum; // 슬롯의 번호 ///슬롯 번호 * 0.5초 씩 배치 유닛이 소환된다
    public bool canUse; //현재 슬롯을 사용할 수 있는가

    public Units ItemReturn() { return slot.Peek(); } // 슬롯에 존재하는 아이템이 뭔지 반환.
    //public bool ItemMax(Units item) { return ItemReturn().MaxCount > slot.Count; } // 겹칠수 있는 한계치를 넘으면.   
    ...
    void Start()
    {
        ...
        Lock = transform.GetChild(1).transform.gameObject;
        // 맨 처음엔 슬롯이 비어있다.
        isSlot = false;
    ...
    public void AddItem(Units item)
    {
        ...
        // 스택에 아이템 추가.
        slot.Push(item);
        UpdateInfo(true, item.DefaultImg, item.UnitName);
    ...
    public void UpdateInfo(bool isSlot, Sprite sprite, string name)
    {

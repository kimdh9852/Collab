...
{
    public Stack<RollingAttackUnits> Aslot;       // 슬롯을 스택으로 만든다.
    public Stack<RollingDefenseUnits> Dslot;       // 슬롯을 스택으로 만든다.
    ...
    public RollingAttackUnits ItemReturnA() { return Aslot.Peek(); } // 슬롯에 존재하는 아이템이 뭔지 반환.  
    public RollingDefenseUnits ItemReturnD() { return Dslot.Peek(); } // 슬롯에 존재하는 아이템이 뭔지 반환.  
    ...
    {
        // 스택 메모리 할당.
        Aslot = new Stack<RollingAttackUnits>();
        Dslot = new Stack<RollingDefenseUnits>();
    ...
    public void AddItemA(RollingAttackUnits item)
    {
        // 스택에 아이템 추가.
        Aslot.Push(item);
    ...
    public void AddItemD(RollingDefenseUnits item)
    {
        // 스택에 아이템 추가.
        Dslot.Push(item);
        unitName = item.UnitName;
